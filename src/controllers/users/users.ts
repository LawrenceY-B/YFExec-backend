...
export const editMembers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const updatedMember = await MemberData.findOneAndUpdate({ _id: id }, update, { new: true });
    if (!updatedMember) {
      return res.status(400).json({ success: false, message: "Couldn't update member" });
    }
    return res.status(200).json({
      success: true,
...
export const removeDuplicates = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (duplicate.length > 0) {
      ...
      return res.status(200).json({ success: true, message: "Duplicate removed" });
    }
    return res.status(200).json({ success: true, message: "No duplicate found", data: duplicate });
  } catch (error) {
    next(error);
  }
};
      ...
      const birthdayMatch = await MemberData.aggregate([
        {
          $match: {
            $expr: {
              $and: [{ $eq: [{ $dayOfMonth: "$DoB" }, day] }, { $eq: [{ $month: "$DoB" }, month] }],
            },
          },
        },
      ]);
...
export const downloadExcel = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.setHeader("Content-Disposition", "attachment; filename=youth_members_with_stats.xlsx");
